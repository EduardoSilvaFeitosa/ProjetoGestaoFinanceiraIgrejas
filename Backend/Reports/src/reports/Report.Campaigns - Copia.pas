...
begin
  ...
  begin
    QryCampaing.SQL.Add(' AND numyear = :numyear ');
    QryTrCampaing.SQL.Add(' AND numyear = :numyear ');
  ...
  begin
    QryCampaing.SQL.Add(' AND idmonth = :idmonth ');
    QryTrCampaing.SQL.Add(' AND nummonth = :idmonth ');
  ...
  begin
    ...
    if LWithBalance then
      QryCampaing.SQL.Add('AND valorP > 0');
  end;

  QryCampaing.SQL.Add('ORDER BY numyear, idmonth');
  QryCampaing.Open();

  if ((QryCampaing.RecordCount = 0) or (QryCampaing.IsEmpty)) then
    Exit(False);

  QryTrCampaing.SQL.Add('ORDER BY fOrder, descricao, dtFato');
  QryTrCampaing.Open();
