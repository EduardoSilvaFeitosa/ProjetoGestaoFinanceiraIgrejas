inherited FrmReportCampaigns: TFrmReportCampaigns
  Caption = 'FrmReportCampaigns'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnBackground: TPanel
    inherited pnHead: TPanel
      inherited lbHeaderTitle: TLabel
        Caption = 'Relat'#243'rio de Campanhas'
        ExplicitWidth = 218
      end
    end
    inherited pnContent: TPanel
      object pnContainerCampaign: TPanel
        Left = 20
        Top = 0
        Width = 407
        Height = 80
        Align = alTop
        BevelEdges = []
        BevelOuter = bvNone
        ParentColor = True
        TabOrder = 1
        object Panel2: TPanel
          Left = 0
          Top = 0
          Width = 407
          Height = 80
          Align = alClient
          BevelEdges = []
          BevelOuter = bvNone
          Padding.Top = 10
          Padding.Bottom = 10
          ParentColor = True
          TabOrder = 0
          object Panel6: TPanel
            Left = 0
            Top = 10
            Width = 407
            Height = 15
            Align = alTop
            BevelEdges = []
            BevelOuter = bvNone
            TabOrder = 0
            object Label1: TLabel
              Left = 0
              Top = 0
              Width = 120
              Height = 15
              Align = alLeft
              AutoSize = False
              Caption = 'Campanhas'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = 6110509
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = []
              ParentFont = False
            end
          end
          object Panel16: TPanel
            AlignWithMargins = True
            Left = 3
            Top = 35
            Width = 401
            Height = 35
            Margins.Top = 10
            Margins.Bottom = 0
            Align = alClient
            BevelEdges = []
            BevelOuter = bvNone
            Padding.Right = 4
            Padding.Bottom = 4
            TabOrder = 1
            object Shape2: TShape
              Left = 0
              Top = 0
              Width = 397
              Height = 31
              Align = alClient
              Brush.Color = 16513014
              Pen.Style = psClear
              Shape = stRoundRect
              Visible = False
              ExplicitLeft = 168
              ExplicitTop = 24
              ExplicitWidth = 65
              ExplicitHeight = 65
            end
            object cbCampaign: TDBLookupComboBox
              Left = 0
              Top = 0
              Width = 397
              Height = 29
              Align = alClient
              BevelEdges = []
              BevelInner = bvNone
              BevelOuter = bvNone
              Color = clWhite
              Ctl3D = True
              DataField = 'id'
              DataSource = dtsCampaign
              Font.Charset = DEFAULT_CHARSET
              Font.Color = 9200741
              Font.Height = -16
              Font.Name = 'Segoe UI'
              Font.Style = []
              KeyField = 'id'
              ListField = 'descricao'
              ListSource = dtsListCampaigns
              ParentCtl3D = False
              ParentFont = False
              TabOrder = 0
              StyleElements = [seFont, seClient]
            end
          end
        end
      end
      object pnContainerMonthYear: TPanel
        Left = 20
        Top = 80
        Width = 407
        Height = 80
        Align = alTop
        BevelEdges = []
        BevelOuter = bvNone
        ParentColor = True
        TabOrder = 2
        ExplicitTop = 0
        object gpMonthYear: TGridPanel
          Left = 0
          Top = 0
          Width = 407
          Height = 78
          Align = alTop
          BevelEdges = []
          BevelOuter = bvNone
          ColumnCollection = <
            item
              Value = 50.000000000000000000
            end
            item
              Value = 50.000000000000000000
            end>
          ControlCollection = <
            item
              Column = 0
              Control = Panel1
              Row = 0
            end
            item
              Column = 1
              Control = Panel11
              Row = 0
            end>
          ParentBackground = False
          ParentColor = True
          RowCollection = <
            item
              Value = 100.000000000000000000
            end>
          TabOrder = 0
          object Panel1: TPanel
            Left = 0
            Top = 0
            Width = 204
            Height = 78
            Align = alClient
            BevelEdges = []
            BevelOuter = bvNone
            Padding.Top = 10
            Padding.Right = 5
            Padding.Bottom = 10
            ParentColor = True
            TabOrder = 0
            object Panel3: TPanel
              Left = 0
              Top = 10
              Width = 199
              Height = 15
              Align = alTop
              BevelEdges = []
              BevelOuter = bvNone
              TabOrder = 0
              object Label2: TLabel
                Left = 0
                Top = 0
                Width = 120
                Height = 15
                Align = alLeft
                AutoSize = False
                Caption = 'M'#234's '
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 6110509
                Font.Height = -11
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
              end
            end
            object Panel4: TPanel
              AlignWithMargins = True
              Left = 3
              Top = 35
              Width = 193
              Height = 33
              Margins.Top = 10
              Margins.Bottom = 0
              Align = alClient
              BevelEdges = []
              BevelOuter = bvNone
              Padding.Right = 4
              Padding.Bottom = 4
              TabOrder = 1
              object Shape1: TShape
                Left = 0
                Top = 0
                Width = 189
                Height = 29
                Align = alClient
                Brush.Color = 16513014
                Pen.Style = psClear
                Shape = stRoundRect
                Visible = False
                ExplicitLeft = 168
                ExplicitTop = 24
                ExplicitWidth = 65
                ExplicitHeight = 65
              end
              object cbMonth: TDBLookupComboBox
                Left = 0
                Top = 0
                Width = 189
                Height = 29
                Align = alClient
                BevelEdges = []
                BevelInner = bvNone
                BevelOuter = bvNone
                Color = clWhite
                Ctl3D = True
                DataField = 'idmonth'
                DataSource = dtsMonths
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 9200741
                Font.Height = -16
                Font.Name = 'Segoe UI'
                Font.Style = []
                KeyField = 'idmonth'
                ListField = 'description'
                ListFieldIndex = 1
                ListSource = dtsListMonths
                ParentCtl3D = False
                ParentFont = False
                TabOrder = 0
                StyleElements = [seFont, seClient]
              end
            end
          end
          object Panel11: TPanel
            Left = 204
            Top = 0
            Width = 203
            Height = 78
            Align = alClient
            BevelEdges = []
            BevelOuter = bvNone
            Padding.Left = 5
            Padding.Top = 10
            Padding.Bottom = 10
            ParentColor = True
            TabOrder = 1
            object Panel12: TPanel
              Left = 5
              Top = 10
              Width = 198
              Height = 15
              Align = alTop
              BevelEdges = []
              BevelOuter = bvNone
              TabOrder = 0
              object Label5: TLabel
                Left = 0
                Top = 0
                Width = 120
                Height = 15
                Align = alLeft
                AutoSize = False
                Caption = 'Ano'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 6110509
                Font.Height = -11
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
              end
            end
            object Panel13: TPanel
              AlignWithMargins = True
              Left = 8
              Top = 35
              Width = 192
              Height = 33
              Margins.Top = 10
              Margins.Bottom = 0
              Align = alClient
              BevelEdges = []
              BevelOuter = bvNone
              Padding.Top = 2
              Padding.Bottom = 2
              TabOrder = 1
              object Shape3: TShape
                Left = 0
                Top = 2
                Width = 192
                Height = 29
                Align = alClient
                Brush.Color = 16513014
                Pen.Style = psClear
                Shape = stRoundRect
                ExplicitLeft = 168
                ExplicitTop = 24
                ExplicitWidth = 65
                ExplicitHeight = 65
              end
              object edtYear: TEdit
                AlignWithMargins = True
                Left = 5
                Top = 9
                Width = 184
                Height = 19
                Margins.Left = 5
                Margins.Top = 7
                Align = alClient
                BevelEdges = []
                BevelInner = bvNone
                BevelOuter = bvNone
                BorderStyle = bsNone
                Color = 16513014
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 9200741
                Font.Height = -13
                Font.Name = 'Segoe UI'
                Font.Style = []
                NumbersOnly = True
                ParentFont = False
                TabOrder = 0
              end
            end
          end
        end
      end
      object pnContainerStatus: TPanel
        Left = 20
        Top = 160
        Width = 407
        Height = 41
        Align = alTop
        BevelEdges = []
        BevelOuter = bvNone
        Padding.Top = 10
        Padding.Right = 25
        ParentColor = True
        TabOrder = 3
        ExplicitTop = 80
        object pnCkbStatus: TPanel
          Left = 0
          Top = 10
          Width = 382
          Height = 23
          Margins.Left = 0
          Margins.Right = 0
          Margins.Bottom = 0
          Align = alTop
          BevelEdges = []
          BevelOuter = bvNone
          ParentBackground = False
          ParentColor = True
          TabOrder = 0
          object lbStatus: TLabel
            AlignWithMargins = True
            Left = 23
            Top = 2
            Width = 249
            Height = 21
            Cursor = crHandPoint
            Margins.Left = 0
            Margins.Top = 2
            Margins.Right = 0
            Margins.Bottom = 0
            Align = alLeft
            Caption = ' Somente campanhas com movimenta'#231#227'o?'
            FocusControl = pnContainerStatus
            Font.Charset = DEFAULT_CHARSET
            Font.Color = 6110509
            Font.Height = -13
            Font.Name = 'Segoe UI'
            Font.Style = []
            ParentFont = False
            OnClick = lbStatusClick
            ExplicitHeight = 17
          end
          object iconCkb: TImage
            Left = 0
            Top = 0
            Width = 23
            Height = 23
            Cursor = crHandPoint
            Align = alLeft
            Center = True
            Picture.Data = {
              
              0000}
            OnClick = lbStatusClick
            ExplicitLeft = 5
            ExplicitHeight = 25
          end
        end
      end
    end
  end
  inherited spvMessages: TSplitView
    inherited cpMessage: TCardPanel
      inherited CardWarning: TCard
        ExplicitWidth = 0
        ExplicitHeight = 48
        inherited Panel5: TPanel
          ExplicitLeft = 0
          ExplicitTop = 0
          ExplicitWidth = 0
          ExplicitHeight = 48
          inherited Panel9: TPanel
            ExplicitLeft = -27
          end
        end
      end
    end
  end
  object mtListCampaigns: TFDMemTable
    IndexFieldNames = 'id'
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 325
    Top = 48
    object mtListCampaignsid: TLargeintField
      FieldName = 'id'
      Origin = 'id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object mtListCampaignsdescricao: TWideStringField
      FieldName = 'descricao'
      Origin = 'descricao'
      Size = 100
    end
  end
  object dtsListCampaigns: TDataSource
    DataSet = mtListCampaigns
    Left = 405
    Top = 50
  end
  object mtCampaign: TFDMemTable
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 325
    Top = 104
    object LargeintField1: TLargeintField
      FieldName = 'id'
      Origin = 'id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
  end
  object dtsCampaign: TDataSource
    DataSet = mtCampaign
    Left = 406
    Top = 101
  end
  object dtsListMonths: TDataSource
    DataSet = mtListMonth
    Left = 404
    Top = 154
  end
  object mtListMonth: TFDMemTable
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 322
    Top = 153
    object mtListMonthidmonth: TIntegerField
      FieldName = 'idmonth'
      Origin = 'idmonth'
    end
    object mtListMonthdescription: TWideStringField
      FieldName = 'description'
      Origin = 'description'
      Size = 8190
    end
  end
  object mtMonths: TFDMemTable
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 323
    Top = 209
    object mtMonthsidmonth: TIntegerField
      FieldName = 'idmonth'
      Origin = 'idmonth'
    end
  end
  object dtsMonths: TDataSource
    DataSet = mtMonths
    Left = 402
    Top = 210
  end
end
