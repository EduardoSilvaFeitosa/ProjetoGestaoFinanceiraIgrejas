inherited FrmReportTithes: TFrmReportTithes
  Caption = 'FrmReportTithes'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnBackground: TPanel
    inherited pnHead: TPanel
      inherited lbHeaderTitle: TLabel
        Caption = 'Relat'#243'rio de D'#237'zimos'
        ExplicitWidth = 184
      end
    end
    inherited pnContent: TPanel
      object pnContainerMemberYear: TPanel
        Left = 20
        Top = 0
        Width = 407
        Height = 80
        Align = alTop
        BevelEdges = []
        BevelOuter = bvNone
        ParentColor = True
        TabOrder = 1
        object gpMonthYear: TGridPanel
          Left = 0
          Top = 0
          Width = 407
          Height = 78
          Align = alTop
          BevelEdges = []
          BevelOuter = bvNone
          ColumnCollection = <
            item
              Value = 70.000000000000000000
            end
            item
              Value = 30.000000000000000000
            end>
          ControlCollection = <
            item
              Column = 0
              Control = Panel2
              Row = 0
            end
            item
              Column = 1
              Control = Panel1
              Row = 0
            end>
          ParentBackground = False
          ParentColor = True
          RowCollection = <
            item
              Value = 100.000000000000000000
            end>
          TabOrder = 0
          object Panel2: TPanel
            Left = 0
            Top = 0
            Width = 285
            Height = 78
            Align = alClient
            BevelEdges = []
            BevelOuter = bvNone
            Padding.Top = 10
            Padding.Right = 5
            Padding.Bottom = 10
            ParentColor = True
            TabOrder = 0
            object Panel6: TPanel
              Left = 0
              Top = 10
              Width = 280
              Height = 15
              Align = alTop
              BevelEdges = []
              BevelOuter = bvNone
              TabOrder = 0
              object Label1: TLabel
                Left = 0
                Top = 0
                Width = 120
                Height = 15
                Align = alLeft
                AutoSize = False
                Caption = 'Membro'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 6110509
                Font.Height = -11
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
              end
            end
            object Panel16: TPanel
              AlignWithMargins = True
              Left = 3
              Top = 35
              Width = 274
              Height = 33
              Margins.Top = 10
              Margins.Bottom = 0
              Align = alClient
              BevelEdges = []
              BevelOuter = bvNone
              Padding.Right = 4
              Padding.Bottom = 4
              TabOrder = 1
              object Shape2: TShape
                Left = 0
                Top = 0
                Width = 270
                Height = 29
                Align = alClient
                Brush.Color = 16513014
                Pen.Style = psClear
                Shape = stRoundRect
                Visible = False
                ExplicitLeft = 168
                ExplicitTop = 24
                ExplicitWidth = 65
                ExplicitHeight = 65
              end
              object cbMember: TDBLookupComboBox
                Left = 0
                Top = 0
                Width = 270
                Height = 29
                Align = alClient
                BevelEdges = []
                BevelInner = bvNone
                BevelOuter = bvNone
                Color = clWhite
                Ctl3D = True
                DataField = 'id'
                DataSource = dtsMember
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 9200741
                Font.Height = -16
                Font.Name = 'Segoe UI'
                Font.Style = []
                KeyField = 'id'
                ListField = 'nome'
                ListSource = dtsListMembers
                ParentCtl3D = False
                ParentFont = False
                TabOrder = 0
                StyleElements = [seFont, seClient]
              end
            end
          end
          object Panel1: TPanel
            Left = 285
            Top = 0
            Width = 122
            Height = 78
            Align = alClient
            BevelEdges = []
            BevelOuter = bvNone
            Padding.Left = 5
            Padding.Top = 10
            Padding.Bottom = 10
            ParentColor = True
            TabOrder = 1
            object Panel3: TPanel
              Left = 5
              Top = 10
              Width = 117
              Height = 15
              Align = alTop
              BevelEdges = []
              BevelOuter = bvNone
              TabOrder = 0
              object Label2: TLabel
                Left = 0
                Top = 0
                Width = 120
                Height = 15
                Align = alLeft
                AutoSize = False
                Caption = 'Ano'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 6110509
                Font.Height = -11
                Font.Name = 'Tahoma'
                Font.Style = []
                ParentFont = False
              end
            end
            object Panel4: TPanel
              AlignWithMargins = True
              Left = 8
              Top = 35
              Width = 111
              Height = 33
              Margins.Top = 10
              Margins.Bottom = 0
              Align = alClient
              BevelEdges = []
              BevelOuter = bvNone
              Padding.Top = 2
              Padding.Bottom = 2
              TabOrder = 1
              object Shape1: TShape
                Left = 0
                Top = 2
                Width = 111
                Height = 29
                Align = alClient
                Brush.Color = 16513014
                Pen.Style = psClear
                Shape = stRoundRect
                ExplicitLeft = 168
                ExplicitTop = 24
                ExplicitWidth = 65
                ExplicitHeight = 65
              end
              object edtYear: TEdit
                AlignWithMargins = True
                Left = 5
                Top = 9
                Width = 103
                Height = 19
                Margins.Left = 5
                Margins.Top = 7
                Align = alClient
                BevelEdges = []
                BevelInner = bvNone
                BevelOuter = bvNone
                BorderStyle = bsNone
                Color = 16513014
                Font.Charset = DEFAULT_CHARSET
                Font.Color = 9200741
                Font.Height = -13
                Font.Name = 'Segoe UI'
                Font.Style = []
                NumbersOnly = True
                ParentFont = False
                TabOrder = 0
              end
            end
          end
        end
      end
      object pnContainerStatus: TPanel
        Left = 20
        Top = 80
        Width = 407
        Height = 41
        Align = alTop
        BevelEdges = []
        BevelOuter = bvNone
        Padding.Top = 10
        Padding.Right = 25
        ParentColor = True
        TabOrder = 2
        object pnCkbStatus: TPanel
          Left = 0
          Top = 10
          Width = 382
          Height = 23
          Margins.Left = 0
          Margins.Right = 0
          Margins.Bottom = 0
          Align = alTop
          BevelEdges = []
          BevelOuter = bvNone
          ParentBackground = False
          ParentColor = True
          TabOrder = 0
          object lbStatus: TLabel
            AlignWithMargins = True
            Left = 23
            Top = 2
            Width = 239
            Height = 21
            Cursor = crHandPoint
            Margins.Left = 0
            Margins.Top = 2
            Margins.Right = 0
            Margins.Bottom = 0
            Align = alLeft
            Caption = ' Somente membros com movimenta'#231#227'o?'
            FocusControl = pnContainerStatus
            Font.Charset = DEFAULT_CHARSET
            Font.Color = 6110509
            Font.Height = -13
            Font.Name = 'Segoe UI'
            Font.Style = []
            ParentFont = False
            OnClick = lbStatusClick
            ExplicitHeight = 17
          end
          object iconCkb: TImage
            Left = 0
            Top = 0
            Width = 23
            Height = 23
            Cursor = crHandPoint
            Align = alLeft
            Center = True
            Picture.Data = {
              
              0000}
            OnClick = lbStatusClick
            ExplicitLeft = 5
            ExplicitHeight = 25
          end
        end
      end
    end
  end
  inherited spvMessages: TSplitView
    inherited cpMessage: TCardPanel
      inherited CardWarning: TCard
        ExplicitWidth = 0
        ExplicitHeight = 48
        inherited Panel5: TPanel
          ExplicitLeft = 0
          ExplicitTop = 0
          ExplicitWidth = 0
          ExplicitHeight = 48
          inherited Panel9: TPanel
            ExplicitLeft = -27
          end
        end
      end
    end
  end
  object mtListMembers: TFDMemTable
    IndexFieldNames = 'id'
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 325
    Top = 48
    object mtListMembersid: TLargeintField
      FieldName = 'id'
      Origin = 'id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object mtListMembersnome: TWideStringField
      DisplayWidth = 100
      FieldName = 'nome'
      Origin = 'nome'
      Size = 30
    end
  end
  object dtsListMembers: TDataSource
    DataSet = mtListMembers
    Left = 405
    Top = 50
  end
  object dtsMember: TDataSource
    DataSet = mtMember
    Left = 406
    Top = 101
  end
  object mtMember: TFDMemTable
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 325
    Top = 104
    object LargeintField1: TLargeintField
      FieldName = 'id'
      Origin = 'id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
  end
end
